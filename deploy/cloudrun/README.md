...
The client editor now hydrates from that saved `standard_pdf` snapshot on document load and persists refreshed `layoutSnapshotJson` through the normal v1 document save path as content changes settle. Reopen/export parity should improve through that shared saved snapshot before any deeper editor-surface rewrite lands.
