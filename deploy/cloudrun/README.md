...
The authenticated web root is now the Forge Console operator shell. It polls `/api/v1/projects/:projectId/console/agents` for the live agent board and sends operator messages through `/api/v1/projects/:projectId/console/chat`, both using system-managed agent personas instead of user-scoped AI settings.
