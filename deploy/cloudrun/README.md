...
The live editor must preserve normal caret placement inside prose blocks. Inspector selection is for atom/structural blocks only; do not reintroduce click handlers that convert ordinary text clicks into top-level node selections.
`print_pdf` exports may compute a format-specific snapshot for rendering, but the persisted `layoutSnapshotJson` on `ProjectDocument` remains the authoritative `standard_pdf` editor snapshot and should not be overwritten by print-only pagination.
Manual page breaks in the live paginated editor should fill the rest of the current page instead of collapsing to a tiny separator. The compact near-blank break behavior is compatibility-only for the legacy page-canvas path.
Current WYSIWYG/export parity bar is page count, block order, page placement, and column placement. Minor line-wrap drift is still tolerated, but page/column drift should be treated as a production bug.
...
The authenticated web root is now the Forge Console operator shell. It polls `/api/v1/projects/:projectId/console/agents` for the live agent board. Non-interviewer operator chat still goes through `/api/v1/projects/:projectId/console/chat` using system-managed agent personas instead of user-scoped AI settings.
The interviewer lane in the Forge Console is now session-backed intake, not advisory chat. The dashboard should hydrate `/api/v1/projects/:projectId/interview/sessions/latest` and use the create/append/lock interview endpoints directly when the interviewer is selected.
Console chat replies now include `responseMode: "model" | "fallback"`. Treat fallback-heavy console behavior as an operational signal that the system-managed model lane is degraded even if the rest of the app still loads.
...
The authenticated Cloud Run smoke now enters autonomous generation through the interviewer contract: create interview session -> lock interview brief -> create generation run with `interviewSessionId`. If deploy smoke starts failing on the old prompt-only path again, treat that as a regression.
