...
The authenticated dashboard at `/` is now the Forge Console operator shell, not a passive project grid. Keep project selection and agent-board polling on the `/api/v1/projects/:projectId/console/*` contract and treat the standalone `dm-forge-console` prototype as retired.
The Forge Console interviewer is not advisory chat anymore. When the interviewer is selected, the dashboard must use the real interview-session contract (`/api/v1/projects/:projectId/interview/sessions/latest`, create, append, lock) and render the persisted interview transcript plus brief state in-place.
...
The Forge Console agent board should synthesize those same autonomous-flow signals into the operator roster: interviewer, writer, D&D expert, layout expert, artist, critic, final editor, printer, plus the Forgemaster coordinator. Console chat remains advisory/operator-facing for the non-interviewer agents and should use system-managed agent personas, not the user’s saved chat model settings.
Console chat replies now carry `responseMode: model | fallback`. Preserve that transport field and surface fallback replies subtly in the UI so the operator can tell when a specialist answered from degraded context instead of a live model call.
...
The Cloud Run redeploy smoke should wait for a short worker stabilization window before creating fresh generation jobs. Old worker revisions can still be draining BullMQ work during rollout; use `WORKER_STABILIZATION_SECONDS` in `scripts/redeploy-cloudrun.sh` instead of starting smoke immediately.
